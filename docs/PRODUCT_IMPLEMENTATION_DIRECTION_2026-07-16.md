...
The first minimal vertical slice now exists in code:

```text
PlanReviewDialog
  -> AgentPlanParser
  -> AgentPlanCommandMapper
  -> ApprovalCard
  -> AgentPipeline
  -> OutputsHub / AuditLog
```

Continue hardening it into a complete workflow:
...
- write-file and local-action permissions are checked,
- approval is visible,
