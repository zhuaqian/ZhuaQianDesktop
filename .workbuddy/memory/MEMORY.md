...
- **raw-csc 引用必须与 csproj 同步**：`build.ps1`/`run-tests.ps1` 的 `$refs`/`$Refs` 必须含 `System.Net.Http.dll` 与 `System.Xml.dll`（csproj 已引，但 raw-`csc` 不会自动带；`GitHostPublisher` 用 `HttpClient` 缺则本地/CI 编译失败）。新增用到框架外程序集的模块时，两处脚本与 csproj 三者都要加。
- **CI 入口 = `.github/workflows/tests.yml`**：push/PR 到 main + tag `v*` 触发；`test` 任务跑 `src/scripts/run-tests.ps1`（编译+测试），`release` 任务跑 `src/build.ps1` 产出 `dist/ZhuaQianDesktop.exe` 并打包上传。改构建脚本后此工作流即本地编译的"真机验证"替代（沙箱禁 `csc`）。build.ps1 现用候选路径解析 csc（兼容本机与 windows-latest），默认输出 `dist/`（被 `/dist/` gitignore）。
...
## 安全约定（凭证 / 会话落盘，2026-07-20 加固）

- **PAT（开源发布）绝不持久化到 `.git/config`**：`GitHostPublisher` 把 PAT 只内嵌进一次性 `git push <url>` 命令行参数；推送后 `remote remove origin` 并 `remote add origin <CleanCloneUrl 无凭证>` 留作本地 origin。绝不做 `remote add origin https://<PAT>@host/...`（否则 `git remote -v` / 备份泄露）。`IGitHost` 新增 `CleanCloneUrl(userName, repoName)` 提供无凭证克隆 URL。
- **浏览器登录会话走 DPAPI 加密落盘**：`BrowserAgentClient.SaveSessionAsync` 经 `Core/SecretProtector`（DPAPI `CurrentUser`，复用 `MainForm.Settings.cs` 的 `ProtectedData` 机制）加密；`StartAsync` 加载时解密到临时文件→`StorageStatePath`→`NewContextAsync` 后立即删临时，异常路径也删。明文临时文件不再落盘。
- **统一原则**：所有本地敏感数据（API key / 会话 cookies+localStorage）一律 DPAPI 密文，不外存明文、不进命令参数/审计日志。新增敏感落盘能力必须复用 `SecretProtector`，不得裸写 `File.WriteAllText` 明文。
- 用户审查明确点名此二处为"当前唯一还开着的具体安全缺口"，现已补；后续重点从"加新能力"切到"敏感数据处理补齐"。
