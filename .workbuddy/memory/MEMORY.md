...
- 行预算：`check-architecture.ps1` 实测主文件 **3616 非空行** = `maxMainLines`（零余量，棘轮同步自提交 `7c70076`），其他 .cs ≤900。
