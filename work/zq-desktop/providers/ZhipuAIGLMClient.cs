...
{
    // 智谱 AI 生态办公系统接入：ChatGLM / GLM 系列（OpenAI 兼容协议）
    public class ZhipuAIGLMClient : IProviderClient
    {
        public string ProviderId { get { return "ZhipuAI"; } }
        ...
        // 智谱开放平台兼容 OpenAI 协议的网关
        public string DefaultEndpoint = "https://open.bigmodel.cn/api/paas/v4";
        ...
        string DoSend(List<Dictionary<string, object>> nativeMessages, ModelInfo model, string apiKey, string endpoint)
        {
            if (string.IsNullOrWhiteSpace(apiKey))
                throw new Exception("Zhipu AI API Key is empty. Get one at " + model.ApiKeyUrl);
            ...
            messages.Add(new Dictionary<string, object> {
                { "role", "system" },
                { "content", "You are ZhuaQian Desktop, a practical local AI work assistant. Be concise and useful. You are integrated with Zhipu AI (ChatGLM) ecosystem. Never claim that you created, saved, exported, attached, moved, deleted, renamed, emailed, ran, clicked, or ended anything locally unless the desktop app explicitly reports a real saved path or execution result. If the user asks for TXT, Word, PowerPoint, or Excel output, provide clean content for that file; the desktop app will handle actual local saving." }
            });
            ...
            using (var wc = new WebClient())
            {
                ...
                catch (WebException ex)
                {
                    ...
                    throw new Exception("Zhipu AI API error for model '" + model.Id + "': " + ex.Message +
                        (detail.Length > 0 ? "\n" + detail : ""));
                }
            }
        }
        ...
        {
            if (resp == null) throw new Exception("Zhipu AI returned empty response.");
            if (!resp.ContainsKey("choices")) throw new Exception("Zhipu AI returned no choices: " + json.Serialize(resp));
            var choices = resp["choices"] as ArrayList;
            if (choices == null || choices.Count == 0) throw new Exception("Zhipu AI returned no choices: " + json.Serialize(resp));
            var choice = choices[0] as Dictionary<string, object>;
            if (choice == null) throw new Exception("Zhipu AI choice is null: " + json.Serialize(resp));
            ...
            throw new Exception("Zhipu AI returned no text: " + json.Serialize(resp));
        }
