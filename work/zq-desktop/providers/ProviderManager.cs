...
namespace ZhuaQianDesktopApp
{
    public class ProviderManager
    {
        ...
        public string LastFallbackNotice;
        ...
        public async Task<string> SendAsync(List<Dictionary<string, object>> nativeMessages)
        {
            ...
            foreach (var fallbackModel in chain)
            {
                try
                {
                    ...
                    {
                        string previous = currentModel != null ? currentModel.DisplayName : "selected model";
                        string reason = errors.Count > 0 ? errors[errors.Count - 1] : "selected model failed";
                        LastFallbackNotice = "Fallback used: " + previous + " -> " + fallbackModel.DisplayName + ". Reason: " + reason;
