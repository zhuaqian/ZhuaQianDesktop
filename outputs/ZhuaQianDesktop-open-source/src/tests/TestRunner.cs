...
class TestRunner
{
    ...
    static void TestAgentPlanParser()
    {
        ...
        var gate = new PermissionGate();
        var pipeline = new AgentPipeline(gate, new AuditLog(Path.Combine(Path.GetTempPath(), "zq_contract_" + Guid.NewGuid().ToString("N") + ".log")), new OutputsHub(Path.GetTempPath()));
        pipeline.Register(new ExportFileExecutor(new OfficeExporter()));
        pipeline.Register(new OrganizeFolderExecutor(Path.GetTempPath()));
        pipeline.Register(new PluginRunExecutor(Path.GetTempPath(), false, 30000, 20000));
        pipeline.Register(new ProcessManageExecutor());
        pipeline.Register(new ComputerControlExecutor());
        pipeline.Register(new RollbackExecutor(Path.GetTempPath()));
        pipeline.Register(new WebSearchExecutor(new WebSearchClient(), 1));
    ...
    static void TestAgentPipeline()
    {
        ...
        try
        {
            ...
            gate.Set("permPluginRun", PermissionLevel.Allow);
            var audit = new AuditLog(Path.Combine(dir, "audit.log"));
            var outputs = new OutputsHub(dir);
            var pipeline = new AgentPipeline(gate, audit, outputs);
            pipeline.Register(new ExportFileExecutor(new OfficeExporter()));
            pipeline.Register(new OrganizeFolderExecutor(dir));
            pipeline.Register(new PluginRunExecutor(dir, false, 30000, 20000));
            pipeline.Register(new ProcessManageExecutor());
            pipeline.Register(new ComputerControlExecutor());
            pipeline.Register(new RollbackExecutor(dir));
            pipeline.Register(new WebSearchExecutor(new WebSearchClient(), 1));
