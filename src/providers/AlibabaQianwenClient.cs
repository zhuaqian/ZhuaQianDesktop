...
{
    // 阿里巴巴生态办公系统接入：通义千问 / DashScope（OpenAI 兼容协议）
    public class AlibabaQianwenClient : IProviderClient
    {
        public string ProviderId { get { return "AlibabaQianwen"; } }
        ...
        // DashScope 兼容 OpenAI 协议的网关
        public string DefaultEndpoint = "https://dashscope.aliyuncs.com/compatible-mode/v1";
        ...
        string DoSend(List<Dictionary<string, object>> nativeMessages, ModelInfo model, string apiKey, string endpoint)
        {
            if (string.IsNullOrWhiteSpace(apiKey))
                throw new Exception("Alibaba DashScope API Key is empty. Get one at " + model.ApiKeyUrl);
            ...
            using (var wc = new WebClient())
            {
                ...
                catch (WebException ex)
                {
                    ...
                    throw new Exception("Alibaba Qianwen API error for model '" + model.Id + "': " + ex.Message +
                        (detail.Length > 0 ? "\n" + detail : ""));
                }
            }
        }
        ...
        {
            if (resp == null) throw new Exception("Alibaba Qianwen returned empty response.");
            if (!resp.ContainsKey("choices")) throw new Exception("Alibaba Qianwen returned no choices: " + json.Serialize(resp));
            var choices = resp["choices"] as ArrayList;
            if (choices == null || choices.Count == 0) throw new Exception("Alibaba Qianwen returned no choices: " + json.Serialize(resp));
            var choice = choices[0] as Dictionary<string, object>;
            if (choice == null) throw new Exception("Alibaba Qianwen choice is null: " + json.Serialize(resp));
            ...
            throw new Exception("Alibaba Qianwen returned no text: " + json.Serialize(resp));
        }
