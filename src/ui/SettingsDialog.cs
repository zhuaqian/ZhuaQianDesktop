...
namespace ZhuaQianDesktopApp
{
    public class SettingsDialog : Form
    {
        ...
        void BuildUI()
        {
            ...
            y += 8;
            y = AddModelSection(y, T("Local Models - advanced, needs Ollama/local hardware", "本地模型 - 进阶，需要 Ollama/本机硬件", "本機模型 - 進階，需要 Ollama/本機硬體"), Color.FromArgb(30, 130, 80), ModelRegistry.Local);
            y += 8;
        ...
        List<ModelInfo> GetRecommendedModels()
        {
            ...
            AddModelIfFound(result, ModelRegistry.Free, "gemini-2.5-flash");
            AddModelIfFound(result, ModelRegistry.Free, "deepseek/deepseek-chat-v3-0324:free");
        ...
        string FormatModelMeta(ModelInfo m)
        {
            ...
            if (string.Equals(m.Endpoint, "Local", StringComparison.OrdinalIgnoreCase))
                parts.Add(T("needs Ollama/local hardware", "需要 Ollama/本机硬件", "需要 Ollama/本機硬體"));
            else if (!m.RequiresApiKey) parts.Add(T("no key needed", "无需密钥", "無需金鑰"));
